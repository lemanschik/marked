import { Marked, Renderer, lexer, parseInline, getDefaults, walkTokens, defaults, setOptions } from 'marked-src/lib/marked.js';
...
describe('marked unit', () => {
  ...
  describe('changeDefaults', () => {
    it('should change global defaults', async() => {
      const { defaults, setOptions } = await import('marked-src/lib/marked.js');
      assert.ok(!defaults.test);
      setOptions({ test: true });
      assert.ok((await import('marked-src/lib/marked.js')).defaults.test);
    });
  });
